...
class item_similarity_recommendation():
    ...
    def get_manufacturer_items(self, productmanufacturer):
        prod_man_list = self.train_data_item[self.train_data_item[self.productmanufacturerid] == productmanufacturer]
        products_for_man_id = list(prod_man_list[self.category].unique())
    ...
    def get_manufacturers(self, category):
        cat_data = self.train_data_item[self.train_data_item[self.category] == category]
        manufacturer_id = set(cat_data[self.productmanufacturerid].unique())
            
        return manufacturer_id
    
    def get_unique_products(self):
        all_products = list(self.train_data_item[self.category].unique())
    ...
    def create_cooccurence_matrix(self,filterproducts, allproducts):
        ...
        for i in range(0,len(allproducts)):
            ...
            category_i_data = self.train_data_item[self.train_data_item[self.category] == allproducts[i]]
            manufacturers_i = set(category_i_data[self.productmanufacturerid].unique())
    ...
    def generate_top_recommendations(self, manid, cooccurence_matrix, allprod, filterprod):
    ...
    def create(self, train_data_item, productmanufacturerid, category):
        self.train_data_item = train_data_item
        self.productmanufacturerid = productmanufacturerid
        self.category = category
    ...
    def recommend(self, manuid):
        ...
        print("no. of unique products in the training set: %d" % len(allprod))
            
            #create a cooccurence matrix
        cooccurence_matrix = self.create_cooccurence_matrix(filterprod,allprod)
            
        #Use cooccurence for recommendations
        df_recommendations = self.generate_top_recommendations(manuid, cooccurence_matrix, allprod, filterprod)
